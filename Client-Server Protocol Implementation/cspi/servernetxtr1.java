...
import java .awt.event.*;
 ...
 class servernetxtr1 extends JFrame 
{
 Date d;
 JTree mytree;
 DefaultMutableTreeNode line1,first ;
...
public servernetxtr1(ServerSocket s1)
{
  ...
  try
  {
  /*InetAddress iaser = InetAddress.getLocalHost();
  
  String nameser = iaser.getHostName();
  sser = new Socket(nameser,1111);  */

  String host="127.0.0.1";
  sser=new Socket(host,6060);
 ...
 FileOutputStream fos7=new FileOutputStream("c:\\ExceptionPage.txt",true);
                                                fos7.write(" ".getBytes());
                                                fos7.write((" The Exception Occured is "+o+" while connecting to the super server ").getBytes());
...
//  sser = new Socket(nameser,1111);
      ...
      bxr=true;
       ...
       f1=false;
       System.out.println("SIZE OF EX : "+ex.size());
     ...
     System.out.println(line);
        ...
        bxr=false; 
		 ...
		 FileOutputStream fos8=new FileOutputStream("c:\\ExceptionPage.txt",true);
                                                fos8.write("\n".getBytes());
                                                fos8.write((" The Exception Occured is "+t+" while connecting to the server ").getBytes());
 ...
 public static void main(String args[])
{
 ...
 try
 {
	 String strAddress=(InetAddress.getLocalHost()).getHostName();
	...
	System.out.println("you have connected  to :"+strservername);
	ss= new Socket(strAddress,6060);
...
//      ops=new PrintStream(s.getOutputStream());
System.out.println("new1 : "+new1);

 

       out2 = new PrintWriter(sser.getOutputStream(), true);
       in2 = new BufferedReader(new InputStreamReader(sser.getInputStream()));
	   out2.println(new1);
	   System.out.println("connected to server");
	  ...
	  /* d=new Date();
       ...
       fos2.write(" ".getBytes());
       fos2.write((" The user "+new1+" connected to the server").getBytes());
       fos2.close();
       ...
       String line="";
        for(int j = 0;j<(ex.size()-1);j++)
        {
              ...
              t[j].sleep(200);
                      ...
                      fos3.write(" ".getBytes());
                      fos3.write((" The Exception Occured is "+s+" while connecting to the server ").getBytes());
                      fos3.close();
			  }
         ...
         op.println(ex.elementAt(j)+"#"+ex.elementAt(j)+'^');
              ...
              t[j].sleep(200);
					  ...
					  FileOutputStream fos4=new FileOutputStream("c:\\ExceptionPage.txt",true);
		         	  fos4.write(" ".getBytes());
				      fos4.write((" The Exception Occured is "+s1).getBytes());
					  fos4.close();
			    }
   ...
   while(true)
   {
     ...
     fos.write(" ".getBytes());
     fos.write(("   The user :"+new1).getBytes());
     fos.write((" Sent the message. Length of the message is "+":"+length1+" Bytes").getBytes());
     fos.write("  ".getBytes());
     fos.write(("Date is "+dat).getBytes());
     fos.write("  ".getBytes());
	 fos.write(("Time is "+tim).getBytes());
     fos.close();
  ...
  else
    {
           if(line!=null)
           {
                ...
                FileOutputStream fos1=new FileOutputStream("c:\\log.txt",true);
     			fos1.write((" The user "+new1+" is sending the file ").getBytes());
                fos1.write((" The length of the file is  :"+l1+" bytes").getBytes());
				fos1.close();
